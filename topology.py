...
class Calculate:
    ...
    def Pairs(self,cmap,aa_data):
        ...
        if cmap["type"] == -1: return
            ...
            if len(self.CA_atn) != 0:
                if len(self.CB_atn) == 0:
                    for r in aa_data.res: group.append(tuple(list(r[:2])+["CA"]))
                else:
            ...
            if len(self.CA_atn) != 0:
                for c in self.CA_atn:
                    pairs += [(self.CA_atn[c][x],self.CA_atn[c][y]) for x in self.CA_atn[c] for y in self.CA_atn[c] if y-x>=cacasep]
                    if len(self.CB_atn) != 0:
                        pairs += [(self.CA_atn[c][x],self.CB_atn[c][y]) for x in self.CA_atn[c] for y in self.CB_atn[c] if y-x>=cacbsep]
                        pairs += [(self.CB_atn[c][x],self.CB_atn[c][y]) for x in self.CB_atn[c] for y in self.CB_atn[c] if y-x>=cbcbsep]
            ...
            weights = np.float_([1.0 for x in pairs])
        if cmap["W"]: self.contacts.append((pairs,distances,weights))
        else: self.contacts.append((pairs,distances,np.ones(weights.shape)))
        return

class Topology:
    def __init__(self,allatomdata,fconst,CGlevel,cmap) -> None:
                ...
                if x.startswith(("CA","CB")):
                    for y in self.excl_volume:
                        if y.startswith(("CA","CB")):
                            C10,C12 = 0.0, ((self.excl_volume[x]+self.excl_volume[y])/2.0)**12
    ...
    def __write_protein_dihedrals__(self,fout,data,chiral):
        ...
        for quads,diheds in data.bb_dihedrals:
            ...
            for x in range(quads.shape[0]):
                ...
                fout.write(" %5d %5d %5d %5d %5d %e %e %d\n"%(quads[x][0],quads[x][1],quads[x][2],quads[x][3],func,diheds[x],Kbb/mfac,3))
